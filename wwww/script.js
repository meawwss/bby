...
/* ============================================================
   ...
   Les couleurs ambiantes / burst sont indépendantes.
   ============================================================ */
function createStardust(ambientLayer, burstLayer, colors) {
  const rand = (a, b) => a + Math.random() * (b - a);

  // Couleurs dérivées : corps légèrement éclairci (éclat), halo = couleur pleine.
  const AMB = colors && colors.ambient ? colors.ambient : '#eaf2ff';
  const BUR = colors && colors.burst ? colors.burst : '#ffffff';
...
window.addEventListener('onWidgetLoad', function (obj) {
  ...
  const CARD_BG_COLOR = fieldData.cardBgColor || '#5e3f3b';
  ...
  const TITLE_COLOR = fieldData.titleColor || '#ac8a86';
  const ARTIST_COLOR = fieldData.artistColor || '#ac8a86';
  ...
  function buildStarOuter(glowHex) {
    return `radial-gradient(circle, ${hexToRgba(glowHex, 0.9)} 0%, ${hexToRgba(glowHex, 0.2)} 25%, transparent 70%)`;
  }
  ...
  // Au lieu de 8 directions (qui laissent des trous aux angles et rasterisent
  // mal en fin), on place N copies sur un cercle de rayon = épaisseur, avec un
  // espacement <= 0.45px. Résultat : équivalent d'une dilatation du glyphe par
  // un disque, soit un stroke à jointures arrondies, net à toutes les tailles.
  function buildOutline(width, color, dropAlpha) {
    const w = Math.max(0, width);
    // Contour quasi nul : on ne garde qu'un léger relief (évite un liseré tacheté).
    if (w < 0.05) return `0 1px 1.5px rgba(0,0,0,${dropAlpha})`;

    const ring = (radius, angleOffset) => {
      const r = radius;
      // Assez de pas pour que deux copies voisines soient à <= 0.45px l'une de
      // l'autre ; borné pour ne pas exploser le nombre d'ombres en gros contour.
    ...
    // Anneau externe (le bord du contour).
    let parts = ring(w, 0);
    // Anneau intermédiaire décalé : remplit l'intérieur des contours épais
    // pour un aplat parfaitement plein (inutile en fin, on l'évite).
    if (w > 1.2) {
      const steps = Math.min(48, Math.max(12, Math.ceil((2 * Math.PI * w) / 0.45)));
      parts = parts.concat(ring(w * 0.55, Math.PI / steps));
    }

    // Relief discret, atténué quand le contour est fin pour ne pas "salir"
    // un liseré léger (c'était la cause du rendu bizarre en valeurs basses).
